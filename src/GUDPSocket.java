...
public class GUDPSocket implements GUDPSocketAPI {
    ...
    public void send(DatagramPacket packet) throws IOException {
        ...
        //之前没有发送到对应地址过，即初始化这个新的window
        if (sendQueueIndex < 0) {
            //create a new GUDPbuffer(window) for this endpoint
    ...
    public class SendThread extends Thread {
        ...
        public void run() {
            ...
            //启动接受ACK的线程，先判断完是否接受到ACK需要滑动窗口，再开始把要发送的数据继续加入滑动窗口中
            ackThread.start();
    ...
    //在GUDPPacket之上封装了一个WrapperPacket类来添加一些属性记录重传次数，是否收到ACK确认，是否发送，结合Instant类来计时
    class wrapperGUDPPacket {
        ...
        //调用instant类now方法记录何时应该重传，相当于定了一个时钟
